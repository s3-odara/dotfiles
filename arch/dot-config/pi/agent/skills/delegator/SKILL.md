...
- Use `specifier`, `planner`, `implementer`, and `review-orchestrator` only when the user requests that phase or the task is already in that phase.
...
- To `implementer` for follow-up fixes: include spec + plan + impl-report + review report and clearly state remaining required changes.
...
5. If validation/review fails, return to `implementer` with the accumulated artifacts and failure details.
