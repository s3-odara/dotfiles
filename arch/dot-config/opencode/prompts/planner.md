...
* Skip `explorer` only when the task is trivial, clearly answerable from already inspected context, unsuitable for that subagent, or blocked.

* Use `internet_researcher` only when uncertainty about external information, versions, security, compatibility, or public documentation materially affects the plan.
...
Call `plan_reviewer` exactly once before outputting the plan.
...
Treat feedback from `plan_reviewer` as suggestions. You decide whether to apply them.
