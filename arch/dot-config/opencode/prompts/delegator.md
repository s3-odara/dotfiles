...
* `explore`: default for local files, config, repo structure, logs, scripts, services, unfamiliar environments.
* `internet_research`: default for technical answers, version-sensitive behavior, docs, compatibility, security, best practices.
...
* `explore`: read-only local discovery
* `internet_research`: external facts/docs/release notes/runtime or security guidance
...
2. Use default `explore`/`internet_research`, then split and delegate to the best specialist (`general` if unmatched).
3. Cross-check/retry on conflict, high risk, or low confidence; integrate one outcome.

For non-trivial implementation prefer `spec` -> `implementer` -> `review_orchestrator`.
